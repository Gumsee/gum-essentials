...
namespace Noise {
    ...
    {
        vec2 tilePos = voronoiPosition(pos, scale, jitter, 0.0f, seed);
    ...
    /*vec3 cracks(vec2 pos, vec2 scale, float jitter, float width, float smoothness, float warp, float warpScale, bool warpSmudge, float smudgePhase, int seed)
    {
    ...
    float cracks(vec2 pos, vec2 scale, float jitter, float width, float smoothness, float warp, float warpScale, bool warpSmudge, int seed)
    {   
