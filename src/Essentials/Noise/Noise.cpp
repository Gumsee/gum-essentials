...
namespace Noise {
    ...
    { 
        float x2 = x * x;
        return x2 * x * (x * (x * 6.0f - 15.0f) + 10.0f); 
    }
    ...
    { 
        float x2 = x * x;
        float u = x2 * x * (x * (x * 6.0f - 15.0f) + 10.0f); 
        float du = 30.0f * x2 * (x * (x - 2.0f) + 1.0f);
        return vec2(u, du);
    }
    vec4 noiseInterpolateDu(const vec2 x) 
    { 
        vec2 x2 = x * x;
        vec2 u = x2 * x * (x * (x * 6.0f - 15.0f) + 10.0f); 
        vec2 du = x2 * 30.0f * (x * (x - 2.0f) + 1.0f);
        return vec4(u.x, u.y, du.x, du.y);
    }
    void noiseInterpolateDu(const vec3 x, vec3& u, vec3& du) 
    { 
        vec3 x2 = x * x;
        u = x2 * x * (x * (x * 6.0f - 15.0f) + 10.0f); 
        du = x2 * 30.0f * (x * (x - 2.0f) + 1.0f);
    }
    ...
    float noise(float pos, float scale, int seed)
    {
        pos *= scale;
        vec2 i = vec2(0.0f, 1.0f) + (float)floor(pos);
        float f = pos - i.x;
        i = vec2::mod(i, vec2(scale)) + seed;

        float u = noiseInterpolate(f);
        return mix(hash1D(i.x), hash1D(i.y), u) * 2.0f - 1.0f;
    }
    ...
    // @param seed Seed to randomize result, range: [0, inf]
    // @return Value of the noise, range: [-1, 1]
    float noise(vec2 pos, vec2 scale, int seed) 
    {
        pos *= scale;
        ...
        float value = mix(a, b, u.x) + (c - a) * u.y * (1.0f - u.x) + (d - b) * u.x * u.y;
    ...
    // @param seed Seed to randomize result, range: [0, inf]
    // @return Value of the noise, range: [-1, 1]
    float noisep(vec2 pos, vec2 scale, float phase, int seed) 
    {
        ...
        vec4 hash = multiHash2D(i);
        hash = vec4::sin(hash * kPI2 + phase) * 0.5f + 0.5f;
        float a = hash.x;
        float b = hash.y;
        float c = hash.z;
        float d = hash.w;
    ...
    vec3 noised(vec2 pos, vec2 scale, int seed) 
    {
        ...
        return vec3(value * 2.0f - 1.0f, derivative.x, derivative.y);
    }
    ...
    // @param seed Seed to randomize result, range: [0, inf]
    // @return x = value of the noise, yz = derivative of the noise, range: [-1, 1]
    vec3 noisepd(vec2 pos, vec2 scale, float phase, int seed) 
    {
        const float kPI2 = 6.2831853f;
        // value noise with derivatives based on Inigo Quilez
        pos *= scale;
    ...
    // @return Value of the noise, range: [-1, 1]
    float noise3d(vec2 pos, vec2 scale, float height, int seed)
    {
        // classic value noise with 3D
        pos *= scale;
        vec3 i = vec3::floor(vec3(pos, height));
        vec3 ip1 = i + vec3(1.0f);
        ...
        r.z = mix(r.x, r.y, u.x);
        r.w = mix(r.z, r.w, u.x);
        return (r.x + (r.y - r.x) * u.x) * 2.0f - 1.0f;
    }
    ...
    vec4 noised3d(vec2 pos, vec2 scale, float time, int seed) 
    {
        ...
        vec4 results = vec4(res1.x, 0.0f, 0.0f, 0.0f) + (vec4(res1.y, res1.y, res1.w, res3.y) - vec4(res1.x, res1.x, res1.z, res3.x)) * vec4(u.x, du.x, du.y, du.z);

        return vec4(results.x * 2.0f - 1.0f, results.y, results.z, results.w);
    }
    ...
    float distanceMetric(vec2 pos, uint metric)
    {
        switch (metric)
        {
            ...
            case 1u:
                // manhattam   
                return vec2::dot(vec2::abs(pos), vec2(1.0f));
            case 2u:
                // chebyshev
    ...
    vec2 multiNoise(vec4 pos, vec4 scale, float phase, vec2 seed) 
    {
        ...
        hash0 = vec4::sin(hash0 * kPI2 + phase) * 0.5f + 0.5f;
        hash1 = vec4::sin(hash1 * kPI2 + phase) * 0.5f + 0.5f;
        ...
        vec2 value = vec2::mix(a, b, vec2(u.x, u.z)) + (c - a) * vec2(u.y, u.w) * (vec2(1.0f) - vec2(u.x, u.z)) + (d - b) * vec2(u.x, u.z) * vec2(u.y, u.w);
        return value * 2.0f - 1.0f;
    }
    ...
    float gridNoiset(vec2 pos, vec2 scale, vec3 translate, float intensity, float time, int seed)
    {
        vec4 n;
        ...
        return (float)pow(t, mix(0.5f, 0.1f, intensity));
    }
    ...
    // @param seed Seed to randomize result, range: [0, inf]
    // @return Value of the noise, range: [0, 1]
    float gridNoise(vec2 pos, vec2 scale, float intensity, float time, int seed)
    {
        vec3 trans0 = (hash3D(vec2(seed)) * 2.0f - 1.0f) * vec3(scale.x, scale.y, scale.x);
        vec4 translate = vec4(0.0f, trans0.x, trans0.y, trans0.z);
    ...
    vec3 dotsNoise(vec2 pos, vec2 scale, float density, float size, float sizeVariation, float roundness, int seed) 
    {
        pos *= scale;
        ...
        float radius = clamp(size + (hash.z * 2.0f - 1.0f) * sizeVariation * 0.5f, 0.0f, 1.0f);
        float value = radius / size;  
        radius = 2.0f / radius;
        f = f * radius - (radius - 1.0f);
        f += vec2(hash.x, hash.y) * (radius - 2.0f);
        f = vec2::pow(vec2::abs(f), mix(20.0f, 1.0f, (float)sqrt(roundness)));

        float u = 1.0f - min(vec2::dot(f, f), 1.0f);
    ...
    vec2 randomLines(vec2 pos, vec2 scale, float count, float width, float jitter, vec2 smoothness, float phase, int seed)
    {
        float strength = jitter * 1.25f;

        // compute gradient
        // TODO: compute the gradient analytically
        vec2 grad;
        vec3 offsets = vec3(1.0f, 0.0f, -1.0f) / 1024.0f;
        ...
        float w = fract(v) / (grad / (2.0f * offsets.x)).length();
        width *= 0.1f;
        smoothness *= width;
        smoothness += max<float>({std::abs(grad.x), std::abs(grad.y)}) * 0.02f;
        
        float d = smoothstep(0.0f, smoothness.x, w) - smoothstep(max<float>({width - smoothness.y, 0.0f}), width, w);
        return vec2(d, (float)std::fmod(floor(v), (double)count));
    }
    ...
    vec4 randomLinesv(vec2 pos, vec2 scale, float count, float width, float jitter, vec2 smoothness, float phase, float colorVariation, int seed)
    {
