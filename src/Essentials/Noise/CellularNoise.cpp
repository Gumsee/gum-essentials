...
namespace Noise {
    ...
    vec2 cellularNoise(vec2 pos, vec2 scale, float jitter, int seed) 
    {       
        pos *= scale;
        vec2 i = vec2::floor(pos);
        vec2 f = pos - i;
        
        const vec3 offset = vec3(-1.0f, 0.0f, 1.0f);
        ...
        // add the last value
        vec2 F1 = vec2(vec2::dot(centerPos, centerPos), 1e+5f);
        F.z = F1.x;
        F.w = F1.y;
        ...
        return vec2::sqrt(f12) * (1.0f / 1.125f);
    }
    ...
    // @param seed Seed to randomize result, range: [0, inf], default: 0.0
    // @return Returns the cellular distances, x = F1, y = F2, range: [0, 1].
    vec2 cellularNoisep(vec2 pos, vec2 scale, float jitter, float phase, int seed) 
    {        
        ...
        // add the last value
        vec2 F2 = vec2(vec2::dot(centerPos, centerPos), 1e+5f);
        F.z = F2.x;
        F.w = F2.y;
    ...
    // @param seed Seed to randomize result, range: [0, inf], default: 0.0
    // @return Returns the cellular distances, x = F1, y = F2, range: [0, 1].
    vec2 cellularNoisem(vec2 pos, vec2 scale, float jitter, float phase, uint metric, int seed) 
    {       
        const float kPI2 = 6.2831853f;
        pos *= scale;
        vec2 i = vec2::floor(pos);
        ...
        // add the last value
        vec2 F1 = vec2(distanceMetric(centerPos, metric), 1e+5f);
        F.z = F1.x;
        F.w = F1.y;
        ...
        vec2 f12 = vec2(min(F.x, F.y), max<float>({F.x, F.y}));
        // normalize: 0.75^2 * 2.0  == 1.125
        return (metric == 0u ? vec2::sqrt(f12) : f12) * (1.0f / 1.125f);
    }
    ...
    vec3 cellularNoised(vec2 pos, vec2 scale, float jitter, int seed) 
    {       
        pos *= scale;
        vec2 i = vec2::floor(pos);
        vec2 f = pos - i;
        ...
        t.x = (float)sqrt(t.x);
        // normalize: 0.75^2 * 2.0  == 1.125
        return  t * vec3(1.0f, -2.0f, -2.0f) * (1.0f / 1.125f);
    }
    ...
    // @param seed Seed to randomize result, range: [0, inf], default: 0.0
    // @return x = value of the noise, yz = derivative of the noise, range: [-1, 1]
    vec3 cellularNoisedp(vec2 pos, vec2 scale, float jitter, float phase, int seed) 
    {       
        const float kPI2 = 6.2831853f;
        pos *= scale;
        vec2 i = vec2::floor(pos);
    ...
    float metaballs(vec2 pos, vec2 scale, float jitter, int seed) 
    {       
        pos *= scale;
        vec2 i = vec2::floor(pos);
        vec2 f = pos - i;
    ...
    float metaballsw(vec2 pos, vec2 scale, float jitter, float width, float smoothness, int seed) 
    {       
    ...
    // @param seed Seed to randomize result, range: [0, inf], default: 0.0
    // @return Returns the metaballs distance from the cell edges, range: [0, 1]
    float metaballsp(vec2 pos, vec2 scale, float jitter, float phase, int seed) 
    {       
        const float kPI2 = 6.2831853f;
        pos *= scale;
        vec2 i = vec2::floor(pos);
    ...
    float metaballswp(vec2 pos, vec2 scale, float jitter, float phase, float width, float smoothness, int seed) 
    {       
    ...
    // @param seed Seed to randomize result, range: [0, inf], default: 0.0
    // @return Returns the metaballs distance from the cell edges, range: [0, 1]
    float metaballspm(vec2 pos, vec2 scale, float jitter, float phase, uint metric, int seed) 
    {       
        const float kPI2 = 6.2831853f;
        pos *= scale;
        vec2 i = vec2::floor(pos);
        ...
        return metric == 0u ? (float)sqrt(d) : d;
    }

    float metaballswpm(vec2 pos, vec2 scale, float jitter, float phase, float width, float smoothness, uint metric, int seed) 
    {       
    ...
    {
        vec3 c0 = cellularNoisem(pos, scale, jitter, phase, metric, 0.0f);
        vec3 c1 = cellularNoisem(pos, scale, jitter, phase, metric, 23.0f);
        c0.x = 1.0f - c0.x;
        c1.x = 1.0f - c1.x;
        if (c0.x > c1.x)
        {
    ...
    {
        vec3 c0 = cellularNoisedp(pos, scale, jitter, phase, 0.0f);
        vec3 c1 = cellularNoisedp(pos, scale, jitter, phase, 23.0f);
        c0.x = 1.0f - c0.x;
        c1.x = 1.0f - c1.x;
        if (c0.x > c1.x)
        {
